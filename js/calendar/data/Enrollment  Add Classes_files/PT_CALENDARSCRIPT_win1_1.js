...
function openDate_win1(e,field,format,bsubmit,cal,firstDayOfWeek)
{
    if (bLoaded == false)
        LoadCalendar();
	...
	dateFormat = format;
	processing_win1(0);
	showCalendar_win1(field, format, '24', true,firstDayOfWeek);
	dateBoxOpen = true;
    //********************************* ICE 1904588000 *********************************
...
function setResult_win1(dt)
{
	var obj = document.win1[dateFieldToUpdate];

	addchg_win1(obj);
	obj.value = pt_formatDate(dt, dateFormat, calendarType);
	PSclearError_win1(obj);
	closeCal();
...
{
    if (obj.id == promptField && "win1" == formName) return;      // Incident 1882152003
	if(_PS_popupCalendar != null)
		closeHandler(_PS_popupCalendar);
	dateBoxOpen=false;
}

function selected_win1(cal, date) {  
	if (cal.dateClicked )  
	{  
...
function showCalendar_win1(id, format, showsTime, showsOtherMonths,firstdayofweek)
 {
	var fld = document.win1[dateFieldToUpdate];  
	var el = document.win1[id];   
	var cal = new PTCalendar(firstdayofweek, null, selected_win1, closeHandler);
	cal.weekNumbers = false; 
	cal.setRange(1900, 2070); 
	cal.create();
	_PS_popupCalendar = cal;
	var string = document.win1[dateFieldToUpdate].value;
