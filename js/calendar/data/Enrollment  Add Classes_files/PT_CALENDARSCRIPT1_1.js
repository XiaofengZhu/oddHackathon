...
PTCalendar.previousYear = function() {
    var cal = window._PS_popupCalendar;
    cal.yearsCombo.selectedIndex--;
    cal.calType == "H" ? selectMonth(cal.month,-1) : selectMonth(cal.date.getMonth(),-1);
};
...
PTCalendar.currentDate = function() {
    var cal = window._PS_popupCalendar;
    var oldYear;
    var oldComboSelectIndex = cal.yearsCombo.selectedIndex;
    
    if(cal.calType == "H")
    {
        oldYear = cal.year;
        cal.setDate(new window.HijriDate());
        cal.monthsCombo.selectedIndex = cal.month;
        cal.yearsCombo.selectedIndex = oldComboSelectIndex + cal.year - oldYear;
        selectMonth(cal.month,0);
    }
    else
    {
        oldYear = cal.date.getYear();
        cal.setDate(new Date());
        cal.monthsCombo.selectedIndex = cal.date.getMonth();
        cal.yearsCombo.selectedIndex = oldComboSelectIndex + cal.date.getYear() - oldYear;
        selectMonth(cal.date.getMonth(),0);
    }
};
PTCalendar.firstDayOfMonth = function() {
    ...
    cal.calType == "H" ? date.day = 1 : date.setDate(1);
    cal.setDate(date);
    cal.dateClicked = false;
    cal.callHandler();
};
PTCalendar.lastDayOfMonth = function() {
    var cal = window._PS_popupCalendar;
    var date = cal.date;

    cal.calType == "H" ? date.day = date.daysOfMonth() : date.setDate(date.getMonthDays(date.Month));
    cal.setDate(date);
    cal.dateClicked = false;
    cal.callHandler();
};
...
PTCalendar._keyEvent = function(ev) {
    ...
    var act = (PTCalendar.is_ie || ev.type == "keydown"),
		K = ev.keyCode;

	if (ev.ctrlKey) {
		switch (K) {
            case 33: // KEY page up
            act && PTCalendar.previousYear();
            break;
            case 34: // KEY page down
            act && PTCalendar.nextYear();
            break;
            case 36: // KEY home
            act && PTCalendar.currentDate();
            break;
		    default:
			return false;
		}
        ...
        case 9:  // KEY tab
        case 27: // KEY esc
        act && PTCalendar.closeWindow();
        break;
        case 33: // KEY page up
        act && PTCalendar.previousMonth();
        break;
        case 34: // KEY page down
        act && PTCalendar.nextMonth();
        break;
        case 35: // KEY end
        act && PTCalendar.lastDayOfMonth();
        break;
        case 36: // KEY home
        act && PTCalendar.firstDayOfMonth();
        break;
	    case 37: // KEY left
	    case 38: // KEY up
	    case 39: // KEY right
		...
		var hasdays = false, iday, dpos = ar_days[i] = [];
			...
			if (!cell.disabled) {
				...
				if(currentDate == SrcDay &&
					cell.calMonth == month &&
                    (this.calType == "T" ? cell.calYear == year - 543 : cell.calYear == year))
				{
					this.CellSel = cell;
